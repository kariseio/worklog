...
- 다음 후보(v0.3.x): Codex 수집기가 편집 파일을 못 뽑는 경우(ai-web-novel 09-15 '파일 0' 이지만 실제 3파일 편집) — 참인 요약 문장이 표와 어긋나는 원인. 규칙 8 사각: 단계 *번호* 매핑('7단계(세 화면 구현)')은 안 조여짐 — '단계 번호는 그날 데이터에 보일 때만' 한 문장. `has_title_source` 가 tool_counts 를 안 봐 발화·파일 없는 MCP 세션은 타임라인에 못 오름. 부분 요약의 질답 원문에 주입 문구(`<recommended_plugins>`)가 그대로 붙음(제목 필터는 통과). 작은 세션 경로는 8줄 상한 미적용. Codex 전용 세션 제목(exec/spawn_agent 도구명). 주간 병합 LLM 출력에 plausible_summary 미적용. Claude compaction 분기 중 uuid 65% 공유 쌍은 echo 규칙(출력 0 + uuid 공유)으로만 접힘.
