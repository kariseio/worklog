...
- 2차 결과: N2 `render::clean_title` 한 통로(피드·문서·타임라인·LLM 신호 전부), N4 0 숨김 + "커밋 0 — 훑은 저장소 N개…", 집중시간 신뢰 표기. N0 `exclude.rs` — 요약 신호에서는 통째로 제거, 문서는 `[비공개 프로젝트]` 집계 한 행. 첫 검증에서 worktree 세션이 빠져나가 `Session.repo_root`(물리 저장소 루트) + 프로젝트 닫힘으로 보강. 실데이터 09-15/16 누출 0.
- 3차 결과: N8 `worklog weekly` — 저장 없음·스키마 v3 그대로. 실DB 에는 신서식 일별 문서가 아직 없어(7/28 v1 문서 1장뿐) 병합·이월·합계는 4장 fixture 로 손계산 대조. M7(파싱 실패율)·V4(실사용) 는 일별 문서가 며칠 쌓인 뒤 측정.
- 게이트 1차(2026-09-18, 09-10~16 무LLM 7일 + 09-14~16 실제 LLM): M1 통과(행 최대 3h52m, 합계 ≤ 활동 구간), M3 통과(86줄 중 0), **M2 실패** — Claude Code resume/fork 가 같은 대화를 새 sessionId 의 새 jsonl 로 재생(루트 메시지 uuid 동일, uuid 91~99% 겹침)해 dedupe_key(session_id) 가 못 접음 → 4차에서 (agent, 루트 uuid) 키 + uuid 포함관계 규칙으로 수정. V2: 09-15·09-16 WRONG 0/24(지어낸 커밋 0, 커밋 0 인 날 완료 단정 0), 09-14 는 claude CLI 240초 고정 타임아웃으로 요약 실패 + 문서가 무LLM 문서와 동일해 실패를 알 수 없음 → 4차에서 timeout_secs 설정(기본 600)·실패 표시·부분 요약 폴백. 부수: NaverWorks 두 캘린더에 같이 있는 일정이 2번 집계 → 4차에서 중복 제거.
- 4차(차단 해소): M2 — `Session.thread_id`(뿌리 메시지 uuid) 1순위 dedupe 키 + 수집 단계 uuid 포함관계(90%) 접기 + '출력 0·질문 상위 포함·uuid 공유' echo 접기 → 09-14/15/16 세션 17→12·27→22·21→18, 중복 그룹 0, 진짜 반복(suda-local ×3) 보존. V2 — `summarizer.timeout_secs`(기본 600) 로 09-14 322초 성공, 실패 사유는 문서·CLI·앱 알림 3곳에 표시, reduce 실패 시 부분 요약. 09-14 재채점 WRONG 2/29(같은 과장 1건 '0~8단계 이식 완료' ×2, 커밋은 0~5단계) → 5차에서 범위 숫자 규칙 추가. 회의 중복(두 캘린더) 제거.
- 5차(마무리): MCP 전용 세션은 tool_counts 로 제목('suda-local · get_workflow 호출 외 2개') → 09-16 맨 폴백 9줄→1줄(Codex). 인사말·거절 응답을 요약 실패로 처리, 질답 폴백 8줄 상한, 규칙 8 범위 숫자 조임 → 09-14 재채점 WRONG 1/27 = 0.56/15줄. 검증 중 발견한 제외 누출 1건(리댁션이 tool_counts 를 안 지워 MCP 도구명이 타임라인에 노출) 수정.
- **게이트 최종(2026-09-18)**: M1 0 · M2 0 · M3 0 · 제외 누출 0 · V2 0.56 WRONG/15줄(≤1) → v0.3.0 태그 가능(사용자 확인 후).
- v0.3.1(2026-09-21): 일지 탭 기본 날짜 오늘(사용자 지적), 요약 신호에서 시스템 주입 발화 생략('(시스템 주입 발화 N개 생략)', 판정은 발화 머리 고정 — 트레이스백 붙여넣기·도구명 인용 질문은 유지), MCP 도구·명령만 있는 세션도 타임라인, 규칙 8 단계 번호 추측 금지. 실데이터 09-15/16 문서는 v0.3.0 게이트 기준선과 바이트 동일, 신호에서 주입 문구 0회.
- 다음 후보(v0.3.x): Codex 수집기가 편집 파일을 못 뽑는 경우(ai-web-novel 09-15 '파일 0' 이지만 실제 3파일 편집) — 참인 요약 문장이 표와 어긋나는 원인. 작은 세션 경로는 8줄 상한 미적용. Codex 전용 세션 제목(exec/spawn_agent 도구명). 주간 병합 LLM 출력에 plausible_summary 미적용. Claude compaction 분기 중 uuid 65% 공유 쌍은 echo 규칙(출력 0 + uuid 공유)으로만 접힘.
